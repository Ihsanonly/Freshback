...
<head>
    ...
    <title>Edit Makanan - FRESHBACK</title>
    <style>
        html,
        body {
            margin: 0;
    ...
    <main>
        <div class="page">
            ...
            <header>
                <div>
                    <h1>Edit Makanan</h1>
                    <p class="muted">Perbarui data {{ $food->name }} di database.</p>
                </div>
            ...
            <div class="form-shell">
                <form method="POST" action="{{ route('foods.update', $food) }}">
                    ...
                    <div class="field form-row">
                        ...
                        <input id="name" name="name" type="text" value="{{ old('name', $food->name) }}" autocomplete="off" autofocus>
                        @error('name')
                    ...
                    <div class="field form-row">
                        ...
                        <input id="quantity" name="quantity" type="text" value="{{ old('quantity', $food->quantity) }}" autocomplete="off">
                        @error('quantity')
                            <p class="error">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="field form-row">
                        ...
                        <input id="purchase_date" name="purchase_date" type="date" value="{{ old('purchase_date', $food->purchase_date?->format('Y-m-d')) }}">
                        @error('purchase_date')
                            <p class="error">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="field form-row">
                        ...
                        <input id="shelf_life_days" name="shelf_life_days" type="number" min="1" value="{{ old('shelf_life_days', $food->shelf_life_days) }}">
                    ...
                    <div class="field form-row">
                        ...
                        <select id="category" name="category">
                            ...
                            @foreach ($categories as $category)
                                <option value="{{ $category }}" @selected(old('category', $food->category) === $category)>{{ $category }}</option>
                            @endforeach
                        </select>
                        @error('category')
                            <p class="error">{{ $message }}</p>
                    ...
                    <div class="field form-row">
                        ...
                        <textarea id="notes" name="notes">{{ old('notes', $food->notes) }}</textarea>
                        @error('notes')
                            <p class="error">{{ $message }}</p>
                        @enderror
                    </div>
